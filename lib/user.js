...
let createNewAmazonUser = function (name) {
  const numberOfContractor = getRandomInt(1, 4)
...
function calcNextStep (userID, stepNumber) {
  ...
  return new Promise( function (resolve, reject) {
    getUserInfo(userID).then( function(result) {
      ...
      else if ( isContractAccepted && stepNumber < 50 )
        nextStep = 50 + result.finally
      
      // 10 <= ~ < 50
      else if ( stepNumber >= 10 && stepNumber < 50 )
      {
      ...
      // 최종결과 -> 최종설문
      else if ( stepNumber == 50 || stepNumber == 51 || stepNumber ==52 )
        nextStep = 53
